...
async function main() {
  ...
  const approvalTools = parseApprovalTools(process.env.GOOGLE_APPROVAL_TOOLS);
      ...
      if (typeof handler === "function") {
        args[hIdx] = async (...hargs: unknown[]) => {
          ...
          try {
            ...
            // The client captures the first mutating call whether or not
            // handleToolCall swallowed the abort into its own error result.
            const composed = api.takeComposed();
            if (composed) {
