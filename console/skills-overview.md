# Skills in OpenClaw + FlatClaw Console

This document explains skills from first principles, how they work in the OpenClaw runtime (`~/openclaw`), and how FlatClaw Console currently exposes them in UX.
...
## 10) FlatClaw Console UX (current behavior)
...
Sorted by most recent creation time in `flatclaw-console/.agent/done`, the latest items are mostly bugfix exec plans (streaming, proxy auth, stale config, cron rollback, etc.).
